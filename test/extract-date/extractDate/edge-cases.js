...
test('parses ISO 8601 date-time format', (t) => {
  ...
  t.deepEqual(extractDate('2000-01-02T00:00'), [{date: '2000-01-02', originalText: '2000-01-02'}]);
});
...
test('extracts date matching multiple formats once', (t) => {
  ...
  t.deepEqual(extractDate('foo bar 02/01/2000', {direction: 'DMY'}), [{date: '2000-01-02', originalText: '02/01/2000'}]);
});

test('full-year formats are used regardless of whether direction defines Y', (t) => {
  t.context.clock.tick(parseDate('2000-01-02', 'yyyy-MM-dd', new Date()).getTime());

  t.deepEqual(extractDate('02/01/2020', {direction: 'DM'}), [{date: '2020-01-02', originalText: '02/01/2020'}]);
});
